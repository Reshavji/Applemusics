...
function Header({ currentSong, toggleSidebar, setCurrentSong }) {
  ...
  useEffect(() => {
    // Retrieve watchlist data from local storage and set it to the state
    ...
    <div className="header-container">
      <div className="header-items">
        ...
        <div className="player">
          <CustomAudioPlayer currentSong={currentSong} />

        </div>

        <div className="list-icon" onClick={handleListIconClick}>
          <ListIcon fontSize="medium" />
        </div>
        <div className={`right-sidebar ${isSidebarExpanded ? "expanded-sidebar" : ""}`}>
          <div className="list-heading">Liked Songs</div>
            ...
            {watchlist.map((song) => (
              <div key={song._id} className="watchlist-song" onDoubleClick={() =>
                ...
                <img
                  ...
                  alt={song.title}
                />
                <p className="watchlist-song-title">{song.title}</p>
                <button
                  className="watchlist-remove-btn"
                  onClick={() => handleRemoveFromWatchlist(song)}
                >
                  Remove
                </button>
              </div>
        ...
        >
          {user ? (
            ...
            <button className="login-btn" onClick={handleLoginButtonClick}>
              <PersonIcon fontSize="small" className="cart-icon" />
              Sign In
            </button>
          )}
